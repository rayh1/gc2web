...
1. **Date determinism — gedq's call, not ours.** A day's candidates are exactly the
   `BIRT` rows in the `gedq anniversary --all-days --json` payload for that `MM-DD`
   key. Year-only, month-only and interpreted (`INT`) dates are therefore absent
   because gedq already excludes them ([R-7]). gc2web deliberately does not
   reimplement that judgement.
...
def _birth_ids_by_day(gedcom_file: str) -> dict[str, list[str]]:
    """The xref ids gedq reports as deterministic births, keyed by `MM-DD`."""
    command = [
        "gedq",
        "anniversary",
        str(gedcom_file),
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"gedq anniversary --all-days failed: {message}")
...
def build_birthday_index(
    ...
    birth_ids_by_day = _birth_ids_by_day(gedcom_file)
    index: dict[str, list[dict[str, str]]] = {}

    for key, xref_ids in birth_ids_by_day.items():
        ...
        index[key] = [entry for _, _, entry in candidates]
